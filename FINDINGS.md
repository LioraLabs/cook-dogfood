...
- [core-bug] test-step dep folding is inconsistent with cook-step dep folding. For a `cook`-step unit, a `$<dep>` body reference folds the upstream's *output content* — early cutoff works: changing `lib`'s command so `build/lib.txt`'s bytes stay byte-identical to a prior cached run left `useref` cached (no re-run). For a `test`-step unit, the same reference folds the upstream's *execution*, not its output content. Minimal repro, same Cookfile as above plus:
  ...
  **CORRECTION (post-fix, per the resolved false-HIT ticket):** the mechanism hypothesis above was disproven during the fix. The local-index recording invariant was sound all along — every write site is on an executed-unit path. What actually happened: bare `cook test` executed the ENTIRE workspace DAG (every non-chore recipe), invisibly, because the test-mode reporter prints only test lines; the typecheck units genuinely ran (real tsc invocations, ~8s hidden wall time) and the later `cached`/`HIT (local)` was a TRUE hit on that hidden execution. "Executed zero typecheck units" was an inference error from the silent output. Fixed by scoping bare/namespace `cook test` to test-bearing recipe roots; regression-pinned.
...
- [core-ergonomics] Header-only recipe dep is ordering-only — confirmed via a controlled tmpdir experiment (independent of, and more rigorous than, the live `apps/web` observation logged below): a `recipe A: B` edge with no `$<B>` placeholder referenced anywhere in A's body contributes *nothing* to A's cache key, only build ordering. Minimal repro:
  ...
  RESOLVED (author): the intended model is exactly what §10.6 specifies — a body placeholder `$<B>` IS the data-dependency declaration (creates the cross-recipe edge with no `: B` header entry needed, per CS-0121, refinable to per-unit edges, folds output content into the cache key), while a `recipe A: B` header dep is a whole-recipe order-only FENCE with zero cache weight. This repo's original sin was reading the header as the data dep; the later "fix" of adding `: $<dep> &&` body refs while *keeping* the header deps produced the redundant-fence antipattern (fence dominates, serializing for nothing). Cleaned in this rework's Cookfile-sweep commit: every body-referenced target dropped from headers; the only survivors are commented deliberate fences (root `build` aggregate's `api.build`/`web.typecheck`/`contracts.gen`, and api `tests: build` to serialize the two dotnet invocations over the shared `obj/` tree). COOK-214 tracks docs+lint (flag a header dep whose target is also body-referenced). Interleaving evidence from the cleanup (cold builds, fresh cache, `.cook/logs/*/events.jsonl`): before-vs-after registered graphs and step interleaving were IDENTICAL (~2.7s wall both ways, every downstream recipe's first unit starting exactly at its upstream's last-unit end) — because the current engine also materialises placeholder-sourced edges at whole-recipe granularity (§10.6 makes per-unit refinement a MAY it doesn't yet exercise), the redundant fences cost nothing *today*; removing them is what makes future per-unit pipelining reachable at all, since refinement is available to placeholder-sourced edges only.
...
- [module:generic] The `: $<dep> &&` body-reference idiom (applied to `apps/web/Cookfile`'s `ui`/`bundle`/`typecheck`/`smoke` bodies in this rework) — RESOLVED (author) framing correction: this is not a "workaround," it is the *intended primary surface* for declaring a data dependency (§10.6/CS-0121: the body placeholder alone creates the cross-recipe edge, folds output content into the cache key, and is the only edge form eligible for per-unit pipelining refinement). The mistake in this rework was pairing the body refs WITH the pre-existing `recipe A: B` header deps — header deps are whole-recipe order-only fences with zero cache weight, so keeping both left redundant fences dominating the schedule; that redundant-fence antipattern is now cleaned in this rework's Cookfile-sweep commit (headers dropped wherever the body references `$<B>`; the few genuine fences kept are commented as such). Observed cost of the redundancy today: none (before/after cold-build interleaving and registered graphs were identical — the current engine keeps placeholder edges recipe-coarse too), but the fences would have blocked any future per-unit refinement. What remains true for modules: a `recipe A: B` header whose target is never body-referenced is still a silent stale-but-green trap for hand-written Cookfiles, and nothing in `cook`'s diagnostics flags it — COOK-214 tracks docs+lint (warn on a header dep that is also body-referenced, i.e. redundant fence, and surface the header-dep-is-a-fence model in docs). Applying this fix made `typecheck`'s fan-out invalidation coarse (any one of `client`/`ui`/`menudata` changing now re-runs all 3 members, since the fan-out step can't cheaply distinguish "does *this* member actually depend on `ui`" without real per-package dependency-graph introspection) — that coarseness is already recorded as the `[module:pnpm]` "Per-package dep topology" finding above; the no-op-reference fix trades a false-negative (silently stale) for over-invalidation (coarse-but-sound), the correct trade for this rework, but it does not replace the real per-member-ingredients fix that finding calls for.
